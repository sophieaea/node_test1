...
capture = cv2.VideoCapture(0) # 0으로 바꿔줘야함 원래 1이었음
capture.set(cv2.CAP_PROP_FRAME_WIDTH, 640)
capture.set(cv2.CAP_PROP_FRAME_HEIGHT, 480)

fourcc = cv2.VideoWriter_fourcc(*'XVID')    # 영상을 기록할 코덱 설정 : 어떤 인코딩을 할건지 정하는 코드
font = ImageFont.truetype('fonts/SCDream6.otf', 20) # 글꼴파일을 불러옴
is_record = False    # 녹화상태는 처음엔 거짓으로 설정 : 녹화상태를 설정하기 위해서 사용함
...
while True:
    ...
    nowDatetime_path = now.strftime('%Y-%m-%d %H_%M_%S') # 파일이름으로는 :를 못쓰기 때문에 따로 만들어줌 -> 에러는 없는데, 오작동이 되면 문제를 찾기 힘든 환장하는 상황이 발생함
    ...
    draw.text(xy=(10, 15),  text="박소이 웹캠 "+nowDatetime, font=font, fill=(255, 255, 255))
    ...
    if key == ord('r') and is_record == False:  # 현재 녹화상태가 아니며 r 키를 누르면
        ...
        # 아래 코드는 video라는 객체를 만들어서 녹화 준비를 해주는 코드임
        video = cv2.VideoWriter("박소이 웹캠 " + nowDatetime_path + ".avi", fourcc, 15, (frame.shape[1], frame.shape[0])) # avi는 확장자를 말하는 것임, 15는 초당프레임을 말함
        ...
        cv2.imwrite("capture " + nowDatetime_path + ".png", frame)  # 파일이름(한글안됨->에러), 이미지 -> 현재 프레임은 프레임 앞에 있는 이름으로 설정해서 작성해줘
