#
#
#   opencv python 코딩
#   노이즈 필터링 코드
#

import cv2
import numpy as np
from opencv_functions import *

# 카메라 영상을 받아올 객체 선언 및 설정(영상 소스, 해상도 설정)
capture = cv2.VideoCapture(0)
capture.set(cv2.CAP_PROP_FRAME_WIDTH, 640)
capture.set(cv2.CAP_PROP_FRAME_HEIGHT, 480)

# 무한루프 -> for in 
while True:
    ret, frame = capture.read() # 카메라로부터 현재 영상을 받아 frame에 저장, 잘 받았다면 ret가 참
    # 영상에 사용할 노이즈 생성. 이런 노이즈를 salt & pepper 노이즈라 함
    noise = np.uint8(np.random.normal(loc=0, scale=0.4, size=[480, 640, 3])) # 이거 노이즈 값을 임의로 가져와서 하는거임
    noised_img = cv2.add(frame, noise)  # 현재 영상에 노이즈를 더함
    # 노이즈 필터링1 : 화면을 흐리게(blur) 뭉게줌
    blur = cv2.blur(noised_img,(5,5))      # cv2.blur(입력이미지,(필터가로,세로크기))-> 얼마나 뿌옇게 할 건지 정하는거임 값을 올리면 더 뿌옇게 된다. 애초에 초상권 방어용으로 안보이게 만드는 필터이다.
    # 노이즈 필터링2 : 가우시안 필터를 이용해 blur. blur 필터에 비해 외곽선이 살아남   
    gaussian = cv2.GaussianBlur(noised_img,(5,5), 0)    # 영상을 덜 흐리게 해준다. 사진 축소 시 선처리용으로 사용
    # 노이즈 필터링3 : bilateral 필터를 적용. 가우시안 필터보다 외곽선을 더 살림
    bilateral = cv2.bilateralFilter(noised_img,9,75,75) # 외곽선을 최대한 살리면서(선명한 유지) 노이즈 제거
    # 노이즈 필터링4 : 메디안 필터. salt & pepper 노이즈 제거에 탁월하며, 외곽선도 잘 살림
    median_filtered = cv2.medianBlur(noised_img,5)    
    '''cv2.imshow("original", frame)       # 원본 영상 출력 -> cv2.imshow('창이름', 출력할이미지) '창이름'의 창을 띄워 이미지를 출력한다.
    cv2.imshow("noised", noised_img)    # 노이즈가 섞인 영상 출력
    cv2.imshow("blur", blur)          # blur 필터를 적용한 영상 출력
    cv2.imshow("gaussian", gaussian)  # 가우시안 필터를 적용한 영상 출력
    cv2.imshow("bilateral", bilateral)    # bilateral 필터를 적용한 영상 출력
    cv2.imshow("median", median_filtered) # 메디안 필터를 적용한 영상 출력 / salt and pepeer 노이즈 제거하는 가장 효과적인 방법
    '''
    # 여러 영상을 하나로 합칠때 사용 -> vconcat은 이미지를 수직으로 붙인다. hconcat은 수평으로 이를 붙여준다.
    total = cv2.hconcat([cv2.vconcat([frame, noised_img]), cv2.vconcat([blur, gaussian]), cv2.vconcat([bilateral, median_filtered])])
    cv2.imshow("total", total)
    if cv2.waitKey(1) == ord('q'):
            break
# capture.release()
# cv2.destroyAllWindows()